...
export async function generateCommonPanel(
  ...
  if (ConfigUtils.validHederaChainID(network)) {
    networkToShow = `Hedera - ${ConfigUtils.getHederaNetwork(network)}`;
  } else {
    networkToShow = ConfigUtils.getOtherNetwork(network);
  }
  const panelToShow = [
    text(`Origin: **${origin}**`),
